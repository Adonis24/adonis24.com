            ...
            <Card sx={{ maxWidth: 1000 }} key={project._id}>
              <Link onClick={() => router.push("./clients")}>
                <CardHeader
                  ...
                  title={project.client.name} // title= {project.client_docs.name}
                  subheader={
                    `Период проекта с ` +
              ...
              <CardContent>
                <Typography variant="body2" color="text.secondary">
                  ...
                  <Link
                    onClick={() =>
                      router.push("./project/" + project.slug, undefined, {
                        shallow: false,
                      })
                    }
                  >
...
export async function getServerSideProps(context) {
  ...
  const searchCategory = category ? category : ""; // ---MongoDb category?{"category_docs.name":category}:{};
  //--Postgres
  const categories = await prisma.category.findMany({})
  const projects = await prisma.project.findMany({
  where:{
      category: {
         name: { 'contains': searchCategory } },
      },
